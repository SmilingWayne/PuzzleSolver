...
class PatchworkParser(BasePuzzleParser):
    
    def __init__(self):
        super().__init__("Patchwork")
    ...
    def parse_puzzle_from_str(self, content: str) -> Optional[Dict]:
        try:
            ...
            region_grid_lines = lines[1 + int(m): ]
            grid = [g.strip().split(" ") for g in grid_lines if g.strip()]
            region_grid = [g.strip().split(" ") for g in region_grid_lines if g.strip()]
