...
class SudokuParser(BasePuzzleParser):
    """(Standard) Sudoku parser"""
    ...
    def parse_puzzle_from_str(self, content: str) -> Optional[Dict]:
        try:
            ...
            if not m.isdigit() or not n.isdigit() or not check_square_num(int(m)) or not check_square_num(int(n)):
    ...
    def parse_solution_from_str(self, content: str) -> Optional[Dict]:
        ...
        try:
            ...
            num_line = lines[0]
            m, n = num_line.strip().split(" ")
            if not m.isdigit() or not n.isdigit() or not n.isdigit() or not check_square_num(int(m)) or not check_square_num(int(n)):
                print("Error: The Puzzle content is malformed. Details: Wrong num_rows or num_cols!")
                return None
            grid_lines = lines[1:1 + int(m)]  
            grid = [g.strip().split(" ") for g in grid_lines if g.strip()]
