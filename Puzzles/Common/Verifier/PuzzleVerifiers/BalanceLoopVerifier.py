...
class BalanceLoopVerifier(BasePuzzleVerifier):
    def __init__(self):
        super().__init__("BalanceLoop")
    ...
    def _verify_safe(self, grid1: Grid, grid2: Grid):
        return all(sorted(grid1.value(position)) == sorted(value) for position, value in grid1 if value != "-")