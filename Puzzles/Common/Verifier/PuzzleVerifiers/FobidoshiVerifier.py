...
class FobidoshiVerifier(BasePuzzleVerifier):
    def __init__(self):
        super().__init__("Fobidoshi")
    ...
    def _verify_safe(self, grid1: Grid, grid2: Grid):
        return all(
            grid1.value(i, j) == grid2.value(i, j) for i in range(grid1.num_rows) for j in range(grid1.num_cols) if grid1.value(i, j) == "o" or grid2.value(i, j) == "o"
        )