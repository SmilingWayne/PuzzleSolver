...
class TerraXVerifier(BasePuzzleVerifier):
    def __init__(self):
        super().__init__("TerraX")
    ...
    def _verify_safe(self, grid1: Grid, grid2: Grid):
        return all(grid1.value(position) == grid2.value(position) for position, value in grid2 if value.isdigit())