...
class PfeilzahlenSolver(PuzzleSolver):
    ...
    def __init__(self, data: dict[str, Any]):
        ...
        # The input data['grid'] is just the numbers (List[List[int]])
        self.input_grid = self._data['grid'] 
        self._check_validity()
        
    def _check_validity(self):
        if len(self.input_grid) != self.num_rows:
            raise ValueError(f"Inconsistent num of rows.")
        if len(self.input_grid[0]) != self.num_cols:
            raise ValueError(f"Inconsistent num of cols.")
    ...
    def _add_number_constraints(self):
        ...
        for i in range(self.num_rows):
            for j in range(self.num_cols):
                target_val = self.input_grid[i][j]
        ...
        for r in range(self.ex_rows):
            for c in range(self.ex_cols):
                if 1 <= r <= self.num_rows and 1 <= c <= self.num_cols:
                    ...
                    sol_matrix[r][c] = str(self.input_grid[r-1][c-1])
                else:
                    # Border: get value from solver
                    val = self.solver.Value(self.x[r, c])
                    sol_matrix[r][c] = int_to_str.get(val, "-")
