...
除了提供这些求解工具，本仓库也包括了一些我自己搜集和爬取的数据集，目前总共包括了超过40类谜题的约8000条数据。具体细节可以参考README文档后面的内容。
...
- 在网络找到的许多谜题的初盘是PDF版本的，但是为了自动求解，毫无疑问需要可以格式化的字符数据，而不是图片或者PDF数据。因此本仓库同样提供了一些易上手的爬虫工具，爬取了一部分容易被结构化的谜题的数据。见[Utils](./Utils/). 目前已经支持40+款谜题的8000+道结构化的谜面数据以及对应的谜题终盘答案。以下给出了目前仓库中提供的数据集清单。注意，数独类谜题的数据集情况在上面已经提供，这里省略。
