...
class TapaHandler(PuzzleFamilyHandler):
    """
    Handler for TAPA puzzles.

    TAPA clues indicate lengths of consecutive shaded cells around the 8 neighbors.
    Clue format: "211" -> [2,1,1], "2?1" -> [2,-2,1], "??" -> [-2,-2]
    """
    ...
    def decode(
        ...
        """Decode TAPA puzzles."""
        qnums_map = self.codecs.tapa.decode(body, num_rows, num_cols)
    ...
    def encode(self, inst: PuzzleInstance) -> str:
        """Encode TAPA PuzzleInstance to puzz.link URL."""
        ...
        body_str = self.codecs.tapa.encode(qnums_map, num_rows * num_cols - 1)
